...
const LOCAL_TEST_DIR = path_1.default.join(process.cwd(), "uploads"); // em dev
...
function sanitizeCnesForMatch(value) {
...
const storage = multer_1.default.diskStorage({
    ...
    filename: (_req, file, cb) => {
        const ext = (path_1.default.extname(file.originalname) || ".pdf").toLowerCase();
        cb(null, `tmp-${Date.now()}-${(0, crypto_1.randomUUID)()}${ext}`);
    },
});
    ...
    try {
        ...
        if (!isValidCNES(cnes)) {
            ...
            if (req.file?.filename) {
                const tmpPath = path_1.default.join(UPLOAD_DIR, req.file.filename);
        ...
        const ext = (path_1.default.extname(req.file.originalname) || ".pdf").toLowerCase();
        ...
        const oldPath = path_1.default.join(UPLOAD_DIR, req.file.filename);
        const newPath = path_1.default.join(UPLOAD_DIR, finalName);
...
router.get("/:filename", (req, res) => {
    try {
        ...
        const filePath = path_1.default.join(UPLOAD_DIR, decoded);
        if (!fs_1.default.existsSync(filePath)) {
            return res.status(404).json({ ok: false, message: "Arquivo não encontrado" });
        }
        res.setHeader("Content-Type", "application/pdf");
        res.setHeader("Content-Disposition", `inline; filename="${path_1.default.basename(decoded)}"`);
...
router.get("/", async (_req, res) => {
    try {
        ...
        const cnesList = Array.from(new Set(baseList.map((x) => x.cnes_match).filter((x) => !!x)));
        let metaByCnes = {};
