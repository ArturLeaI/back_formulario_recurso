...
async function criarAcaoVagasFormularioSemAuth(req, res) {
    ...
    try {
        const { tipoAcao: tipoAcaoRaw, motivoDescredenciar, ufSelecionada, municipioSelecionado, municipio_id, cursos, gestorId, 
        // ✅ para desistir
        ...
        // =========================
        const gestorCheck = await client.query(`SELECT id, cpf, nome, email FROM recursos.gestores WHERE id = $1`, [Number(gestorId)]);
        ...
        if (municipio_id != null && String(municipio_id).trim() !== "") {
            ...
            if (!Number.isFinite(mid))
                return rollbackAndReturn(400, { ok: false, error: "municipio_id inválido" });
            const r = await client.query(`SELECT id, estado_id FROM recursos.municipios WHERE id = $1`, [mid]);
            if (r.rows.length === 0)
                return rollbackAndReturn(400, { ok: false, error: "Município (municipio_id) não encontrado" });
        ...
        `, [municipioNome]);
            if (r.rows.length === 0)
                return rollbackAndReturn(400, { ok: false, error: "Município não encontrado" });
            municipioId = r.rows[0].id;
        }
        if (!municipioId) {
      ...
      `, [municipioId]);
        ...
        // =========================
        const getEstByCnes = async (cnesValue) => {
            const r = await client.query(`
        ...
        for (const curso of cursosArr) {
            ...
            if (!cursoId) {
                ...
                if (!nomeCurso)
                    return rollbackAndReturn(400, { ok: false, error: "Curso sem id/nome válido" });
                const r = await client.query(`
          SELECT id
        ...
        `, [Number(gestorId), tipoAcao, ufNorm, municipioId, estabelecimentoId, cursoId, quantidade]);
            ...
            // atualiza vagas
            const deveSomar = tipoAcao === "AUMENTAR_VAGAS" ||
